...
class Land extends Group {
    ...
    topography(u, v) {
        let x = u * this.params.width - this.params.width / 2;
        let z = v * this.params.height - this.params.height / 2;
        ...
        let p = Perlin.fBM(x, 0, z, s.init_freq, s.oct, s.lac, s.gain, s.init_amp) * s.amp;
        let y = p + g;
        if (y > i.thresholdMin) {
            let alpha = (y - i.thresholdMin) / (i.thresholdMax - i.thresholdMin);
            let noise = (Perlin.fBM(x, 0, z, i.init_freq, i.oct, i.lac, i.gain, i.init_amp) + 1) / 2 * i.amp;
    ...
    updateGeometry() {
        ...
        for (let u = 0; u < w; u += 1) {
            for (let v = 0; v < h; v += 1) {
                let i = index(u, v);
                let [x, z] = this.uvToXZ(u, v);
                let y = this.topography(u / w, v / h);
